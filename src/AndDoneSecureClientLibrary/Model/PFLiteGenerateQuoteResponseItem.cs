/*
 * Secure API
 *
 * Secure API
 *
 * The version of the OpenAPI document: 2.3
 * Generated by: https://github.com/openapitools/openapi-generator.git
 */


using System;
using System.Collections;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using System.Linq;
using System.IO;
using System.Runtime.Serialization;
using System.Text;
using System.Text.RegularExpressions;
using Newtonsoft.Json;
using Newtonsoft.Json.Converters;
using Newtonsoft.Json.Linq;
using System.ComponentModel.DataAnnotations;
using OpenAPIDateConverter = AndDoneSecureClientLibrary.Client.OpenAPIDateConverter;

namespace AndDoneSecureClientLibrary.Model
{
    /// <summary>
    /// PFLiteGenerateQuoteResponseItem
    /// </summary>
    [DataContract(Name = "PFLiteGenerateQuoteResponse_item")]
    public partial class PFLiteGenerateQuoteResponseItem : IValidatableObject
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="PFLiteGenerateQuoteResponseItem" /> class.
        /// </summary>
        /// <param name="quoteKey">quoteKey.</param>
        /// <param name="premium">premium.</param>
        /// <param name="downAmount">downAmount.</param>
        /// <param name="amountFinanced">amountFinanced.</param>
        /// <param name="financeCharge">financeCharge.</param>
        /// <param name="totalPayments">totalPayments.</param>
        /// <param name="paymentAmount">paymentAmount.</param>
        /// <param name="docStamp">docStamp.</param>
        /// <param name="firstDueDate">firstDueDate.</param>
        /// <param name="apr">apr.</param>
        /// <param name="installments">installments.</param>
        /// <param name="batchId">batchId.</param>
        /// <param name="paymentsRetained">paymentsRetained.</param>
        /// <param name="paymentRetainedAmount">paymentRetainedAmount.</param>
        /// <param name="unsignedPFAUrl">unsignedPFAUrl.</param>
        /// <param name="retailAgentRegisterLoginURL">retailAgentRegisterLoginURL.</param>
        /// <param name="eSignResult">eSignResult.</param>
        /// <param name="errors">errors.</param>
        /// <param name="pfa">pfa.</param>
        /// <param name="electronicSignatureURL">electronicSignatureURL.</param>
        /// <param name="offerAutoPay">offerAutoPay.</param>
        public PFLiteGenerateQuoteResponseItem(int quoteKey = default(int), float premium = default(float), float downAmount = default(float), float amountFinanced = default(float), float financeCharge = default(float), float totalPayments = default(float), float paymentAmount = default(float), float docStamp = default(float), string firstDueDate = default(string), float apr = default(float), int installments = default(int), string batchId = default(string), int paymentsRetained = default(int), float paymentRetainedAmount = default(float), string unsignedPFAUrl = default(string), string retailAgentRegisterLoginURL = default(string), PFLiteGenerateQuoteResponseItemESignResult eSignResult = default(PFLiteGenerateQuoteResponseItemESignResult), string errors = default(string), string pfa = default(string), string electronicSignatureURL = default(string), bool offerAutoPay = default(bool))
        {
            this.QuoteKey = quoteKey;
            this.Premium = premium;
            this.DownAmount = downAmount;
            this.AmountFinanced = amountFinanced;
            this.FinanceCharge = financeCharge;
            this.TotalPayments = totalPayments;
            this.PaymentAmount = paymentAmount;
            this.DocStamp = docStamp;
            this.FirstDueDate = firstDueDate;
            this.Apr = apr;
            this.Installments = installments;
            this.BatchId = batchId;
            this.PaymentsRetained = paymentsRetained;
            this.PaymentRetainedAmount = paymentRetainedAmount;
            this.UnsignedPFAUrl = unsignedPFAUrl;
            this.RetailAgentRegisterLoginURL = retailAgentRegisterLoginURL;
            this.ESignResult = eSignResult;
            this.Errors = errors;
            this.Pfa = pfa;
            this.ElectronicSignatureURL = electronicSignatureURL;
            this.OfferAutoPay = offerAutoPay;
        }

        /// <summary>
        /// Gets or Sets QuoteKey
        /// </summary>
        [DataMember(Name = "quoteKey", EmitDefaultValue = false)]
        public int QuoteKey { get; set; }

        /// <summary>
        /// Gets or Sets Premium
        /// </summary>
        [DataMember(Name = "premium", EmitDefaultValue = false)]
        public float Premium { get; set; }

        /// <summary>
        /// Gets or Sets DownAmount
        /// </summary>
        [DataMember(Name = "downAmount", EmitDefaultValue = false)]
        public float DownAmount { get; set; }

        /// <summary>
        /// Gets or Sets AmountFinanced
        /// </summary>
        [DataMember(Name = "amountFinanced", EmitDefaultValue = false)]
        public float AmountFinanced { get; set; }

        /// <summary>
        /// Gets or Sets FinanceCharge
        /// </summary>
        [DataMember(Name = "financeCharge", EmitDefaultValue = false)]
        public float FinanceCharge { get; set; }

        /// <summary>
        /// Gets or Sets TotalPayments
        /// </summary>
        [DataMember(Name = "totalPayments", EmitDefaultValue = false)]
        public float TotalPayments { get; set; }

        /// <summary>
        /// Gets or Sets PaymentAmount
        /// </summary>
        [DataMember(Name = "paymentAmount", EmitDefaultValue = false)]
        public float PaymentAmount { get; set; }

        /// <summary>
        /// Gets or Sets DocStamp
        /// </summary>
        [DataMember(Name = "docStamp", EmitDefaultValue = false)]
        public float DocStamp { get; set; }

        /// <summary>
        /// Gets or Sets FirstDueDate
        /// </summary>
        [DataMember(Name = "firstDueDate", EmitDefaultValue = false)]
        public string FirstDueDate { get; set; }

        /// <summary>
        /// Gets or Sets Apr
        /// </summary>
        [DataMember(Name = "apr", EmitDefaultValue = false)]
        public float Apr { get; set; }

        /// <summary>
        /// Gets or Sets Installments
        /// </summary>
        [DataMember(Name = "installments", EmitDefaultValue = false)]
        public int Installments { get; set; }

        /// <summary>
        /// Gets or Sets BatchId
        /// </summary>
        [DataMember(Name = "batchId", EmitDefaultValue = false)]
        public string BatchId { get; set; }

        /// <summary>
        /// Gets or Sets PaymentsRetained
        /// </summary>
        [DataMember(Name = "paymentsRetained", EmitDefaultValue = false)]
        public int PaymentsRetained { get; set; }

        /// <summary>
        /// Gets or Sets PaymentRetainedAmount
        /// </summary>
        [DataMember(Name = "paymentRetainedAmount", EmitDefaultValue = false)]
        public float PaymentRetainedAmount { get; set; }

        /// <summary>
        /// Gets or Sets UnsignedPFAUrl
        /// </summary>
        [DataMember(Name = "unsignedPFAUrl", EmitDefaultValue = false)]
        public string UnsignedPFAUrl { get; set; }

        /// <summary>
        /// Gets or Sets RetailAgentRegisterLoginURL
        /// </summary>
        [DataMember(Name = "retailAgentRegisterLoginURL", EmitDefaultValue = false)]
        public string RetailAgentRegisterLoginURL { get; set; }

        /// <summary>
        /// Gets or Sets ESignResult
        /// </summary>
        [DataMember(Name = "eSignResult", EmitDefaultValue = false)]
        public PFLiteGenerateQuoteResponseItemESignResult ESignResult { get; set; }

        /// <summary>
        /// Gets or Sets Errors
        /// </summary>
        [DataMember(Name = "errors", EmitDefaultValue = false)]
        public string Errors { get; set; }

        /// <summary>
        /// Gets or Sets Pfa
        /// </summary>
        [DataMember(Name = "pfa", EmitDefaultValue = false)]
        public string Pfa { get; set; }

        /// <summary>
        /// Gets or Sets ElectronicSignatureURL
        /// </summary>
        [DataMember(Name = "electronicSignatureURL", EmitDefaultValue = false)]
        public string ElectronicSignatureURL { get; set; }

        /// <summary>
        /// Gets or Sets OfferAutoPay
        /// </summary>
        [DataMember(Name = "offerAutoPay", EmitDefaultValue = true)]
        public bool OfferAutoPay { get; set; }

        /// <summary>
        /// Returns the string presentation of the object
        /// </summary>
        /// <returns>String presentation of the object</returns>
        public override string ToString()
        {
            StringBuilder sb = new StringBuilder();
            sb.Append("class PFLiteGenerateQuoteResponseItem {\n");
            sb.Append("  QuoteKey: ").Append(QuoteKey).Append("\n");
            sb.Append("  Premium: ").Append(Premium).Append("\n");
            sb.Append("  DownAmount: ").Append(DownAmount).Append("\n");
            sb.Append("  AmountFinanced: ").Append(AmountFinanced).Append("\n");
            sb.Append("  FinanceCharge: ").Append(FinanceCharge).Append("\n");
            sb.Append("  TotalPayments: ").Append(TotalPayments).Append("\n");
            sb.Append("  PaymentAmount: ").Append(PaymentAmount).Append("\n");
            sb.Append("  DocStamp: ").Append(DocStamp).Append("\n");
            sb.Append("  FirstDueDate: ").Append(FirstDueDate).Append("\n");
            sb.Append("  Apr: ").Append(Apr).Append("\n");
            sb.Append("  Installments: ").Append(Installments).Append("\n");
            sb.Append("  BatchId: ").Append(BatchId).Append("\n");
            sb.Append("  PaymentsRetained: ").Append(PaymentsRetained).Append("\n");
            sb.Append("  PaymentRetainedAmount: ").Append(PaymentRetainedAmount).Append("\n");
            sb.Append("  UnsignedPFAUrl: ").Append(UnsignedPFAUrl).Append("\n");
            sb.Append("  RetailAgentRegisterLoginURL: ").Append(RetailAgentRegisterLoginURL).Append("\n");
            sb.Append("  ESignResult: ").Append(ESignResult).Append("\n");
            sb.Append("  Errors: ").Append(Errors).Append("\n");
            sb.Append("  Pfa: ").Append(Pfa).Append("\n");
            sb.Append("  ElectronicSignatureURL: ").Append(ElectronicSignatureURL).Append("\n");
            sb.Append("  OfferAutoPay: ").Append(OfferAutoPay).Append("\n");
            sb.Append("}\n");
            return sb.ToString();
        }

        /// <summary>
        /// Returns the JSON string presentation of the object
        /// </summary>
        /// <returns>JSON string presentation of the object</returns>
        public virtual string ToJson()
        {
            return Newtonsoft.Json.JsonConvert.SerializeObject(this, Newtonsoft.Json.Formatting.Indented);
        }

        /// <summary>
        /// To validate all properties of the instance
        /// </summary>
        /// <param name="validationContext">Validation context</param>
        /// <returns>Validation Result</returns>
        IEnumerable<ValidationResult> IValidatableObject.Validate(ValidationContext validationContext)
        {
            yield break;
        }
    }

}
